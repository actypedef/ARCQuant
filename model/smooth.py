...
def smooth_lm(model, scales, alpha=0.5):
    for name, module in model.named_modules():
        if name.startswith("model."):
            layer_name = name.replace("model.", "", 1)
        if isinstance(module, OPTDecoderLayer):
            ...
            fc1 = module.fc1
            fc1_input_scales = scales[layer_name + ".fc1"]
            smooth_ln_fcs(ffn_ln, fc1, fc1_input_scales, alpha)
        elif isinstance(module, BloomBlock):
            attn_ln = module.input_layernorm
            qkv = module.self_attention.query_key_value
            qkv_input_scales = scales[layer_name + ".self_attention.query_key_value"]
            smooth_ln_fcs(attn_ln, qkv, qkv_input_scales, alpha)

            ffn_ln = module.post_attention_layernorm
            fc1 = module.mlp.dense_h_to_4h
            fc1_input_scales = scales[layer_name + ".mlp.dense_h_to_4h"]
            smooth_ln_fcs(ffn_ln, fc1, fc1_input_scales, alpha)
            ...
            qkv_input_scales = scales[layer_name + ".self_attn.q_proj.input"]
            smooth_ln_fcs_llama_like(attn_ln, qkv, qkv_input_scales, alpha)

            ffn_ln = module.post_attention_layernorm  # feed forward norm
            fcs = [module.mlp.gate_proj, module.mlp.up_proj]
            fcs_input_scales = scales[layer_name + ".mlp.gate_proj.input"]
            ...
            fcs_input_scales = scales[layer_name + ".block_sparse_moe.gate"]
